...
from partial_aggr_job_mult_gpus import PartialAggrJobMultipleGPUs
import os
...
def parse_config(file_path):
    ...
    for line in lines:
        ...
        if m:
            ...
            if len(params) < 6:
                raise ValueError(f"Client {client_id} line must have 6 comma-separated values. Got: {params}")

            freeze_criterion = params[0].lower()
            freeze_value = float(params[1]) if params[1] != "" else 0.0
            qb=int(params[2])
            lr = float(params[3]) if params[3] != "" else None
            epochs = int(params[4]) if params[4] != "" else None
            batch_size = int(params[5]) if params[5] != "" else None
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        if n_total_gpus < 2:
            print(f"Warning: Only {n_total_gpus} GPU(s) available. Assigning 1 GPU per client.")
            gpus_per_client = 1
        else:
            gpus_per_client = 2
            print(f"Found {n_total_gpus} GPUs. Will assign 2 GPUs per client.")
            if n_total_gpus % 2 != 0:
                print("Warning: Odd number of GPUs. The last GPU will not be paired.")
    else:
    ...
    job = PartialAggrJobMultipleGPUs(
        n_clients=n_clients,
        num_rounds=num_rounds,
        #initial_model=model,
    )

    train_script = "./fl/main2_fl.py"
    ...
    job.simulator_run("/tmp/nvflare/jobs/workdir", gpu="(0_1)")
...
#MultGPUJob -> Custom simulator initialization using CustomSimulatorRunner -> CustomScriptRunner uses CustomSimulatorClientRunner