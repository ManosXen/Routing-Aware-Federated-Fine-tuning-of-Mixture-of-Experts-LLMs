...
class PartialClientTrainer(Executor):
    def __init__(
        ...
        self.current_lora = []
        self.trainable = []
        self.converged_experts=[]
        self.model = None
        self.tokenizer = None
        self.sorted_indx = None
        self.effective_batch_size=32

        print("="*20)
        print(f"Init Called! Save file: {save_file_aux}")
        print("="*20)
    ...
    def dataset_load(self, client_name):
        ...
        ###DEBUG
        # num_range = range(0, len(dataset))
        # sel = random.sample(num_range, 450)
        # dataset = dataset.select(sel)
    ...
    def execute(self, task_name: str, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal) -> Shareable:
        ...
        try:
            ...
            state_file = f"/files/jetson_impl/fedavg_aggregation/fl_swap/temp/{self.save_file_aux}_{fl_ctx.get_identity_name()}_state.pkl"
            ...
            if current_round > 0:
                if os.path.exists(state_file):
                    ...
                    self.sorted_indx=saved_state.get('sorted_indx', [])
                    self.converged_experts = saved_state.get('converged_experts', [])
        ...
        try:
            ...
            if current_round == 0:
                ...
                if self.fc == "act_freeze":
                    self.trainable, sorted_indx = freeze_experts_activations(
                        self.model, self.fr_or_thr, self.train_dataset, self.prompt_fn,
                        self.rank, self.batch_size, current_round
                    )
                elif self.fc == "esft":
                    self.trainable, sorted_indx = freeze_experts_esft(
                        self.model, self.fr_or_thr, self.train_dataset, self.prompt_fn,
                        self.rank, self.batch_size, current_round
                    )
                else: # grad_freeze
                    self.trainable, sorted_indx = freeze_experts_gradients(
                        self.model, self.fr_or_thr, self.train_dataset, self.prompt_fn,
                        self.rank, current_round
                    )
                    for p in self.model.parameters():
                        p.grad = None

                self.sorted_indx=sorted_indx.cpu().tolist()
                tr = sum(len(x) for x in self.trainable)
                self.sorted_indx=self.sorted_indx[:-tr]
                ...
                tr_prev = sum(len(x) for x in self.trainable)
                print(f"Client {client_name} trainable params in round {current_round-1}: {tr_prev}")
                print(f"Client {client_name} converged experts in round {current_round-1}: {self.converged_experts}\nClient {client_name} trainable experts in round {current_round-1}: {self.trainable}")
                converged_experts = fl_input.meta.get("converged_experts")
                hard_converged_experts = fl_input.meta.get("hard_converged_experts")

                if len(hard_converged_experts) > 0:
                    self.sorted_indx = [x for x in self.sorted_indx if x not in hard_converged_experts]

                experts_to_drop = set(converged_experts) | set(hard_converged_experts)
                converged_trainable_experts = 0

                if len(experts_to_drop) > 0:
                    converged_trainable_experts=0
                    for gl_exp in experts_to_drop:
                        layer_idx=gl_exp//64
                        exp_idx=gl_exp%64
                        self.converged_experts.append(gl_exp)
                        if exp_idx in self.trainable[layer_idx]:
                            converged_trainable_experts+=1
                            self.trainable[layer_idx].remove(exp_idx)

                    added_experts = []
                    if converged_trainable_experts > 0:
                                        
                                        num_to_add = converged_trainable_experts

                                        if len(self.sorted_indx) == 0:
                                            print(f"[WARNING] Client {client_name}: sorted_indx is EMPTY! Cannot swap in new experts.")
                                        
                                        elif len(self.sorted_indx) < num_to_add:
                                            print(f"[WARNING] Client {client_name}: Only {len(self.sorted_indx)} experts left. Taking all of them.")
                                            added_experts = self.sorted_indx[:]
                                            self.sorted_indx = []
                                        
                                        else:
                                            added_experts = self.sorted_indx[-num_to_add:]
                                            self.sorted_indx = self.sorted_indx[:-num_to_add]

                    print(f"Client {client_name} Added experts {added_experts}, trainable experts {self.trainable}\n", flush=True)
                    for gl_exp in added_experts:
                        layer_idx=gl_exp//64
                        exp_idx=gl_exp%64
                        self.trainable[layer_idx].append(exp_idx)
                    
                    print(f"Client {client_name} New trainable: {self.trainable}")

                tr = sum(len(x) for x in self.trainable)
                #print(f"Client {client_name} trainable params in round {current_round}: {tr}")
                print(f"Client {client_name} converged experts in round {current_round}: {self.converged_experts}\nClient {client_name} trainable experts in round {current_round}: {self.trainable}")
                print(f"Hard Converged Experts in round {current_round}: {hard_converged_experts}", flush=True)
                print(f"Client {client_name} trainable params in round {current_round-1}: {tr_prev}\nClient {client_name} trainable params in round {current_round}: {tr}")
                ...
                self.model, self.tokenizer = build_model(
                    ...
                    converged_experts=self.converged_experts,
