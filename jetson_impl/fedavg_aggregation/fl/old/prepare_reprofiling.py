...
def add_lora(qlinear, rank=4, alpha=8, dropout=0.05):
    ...
    lora_layer = LoraLinear(
        adapter_name="default",  # <-- Explicitly name the adapter
        in_features=in_features,   # <-- Pass in_features by keyword
        out_features=out_features, # <-- Pass out_features by keyword
        base_layer=qlinear,
        r=rank,
        lora_alpha=alpha,
