...
class PartialAggController(ModelController):
    ...
    def aggregate_fn(results: List[FLModel]) -> FLModel:
        ...
        aggr_params, lora_params, current_round_loss, client_times, alloc, res, tr= aggr_helper.get_result()
        ...
        return aggr_result, current_round_loss, rec_size, client_times, alloc, res, tr
    ...
    def aggregate(self, results: List[FLModel], aggregate_fn=None) -> FLModel:
        ...
        try:
            aggr_result, current_round_loss, rec, times, alloc, res, tr= aggregate_fn(results)
    ...
    def check_convergence(self, last_rel_update):
    ...
    def rel_update_calc_and_check_convergence(self, aggr_result):
        ...
        if self.current_round>=self.start_round+self.patience+1:
            return self.check_convergence(expert_rel_updates)
        else:
            return []
    ...
    def run(self) -> None:
        ...
        for self.current_round in range(self.start_round, self.start_round + self.num_rounds):
            ...
            if self.current_round==self.start_round:
                ...
                send_result = FLModel(
                    ...
                    meta={"lora_params": numpy_lora_params, "converged_experts" : converged_experts}
                )
            ...
            save_path = f"/files/jetson_impl/fedavg_aggregation/fl_adaptive_freeze/stats/{save_file}_params_{self.current_round}.pt" 
            print(f"Params save file: {save_path}", flush=True)
            torch.save(self.model_params, save_path)

            end_time=time.time()

        stats_save_path = f"/files/jetson_impl/fedavg_aggregation/fl_adaptive_freeze/stats/training_stats_{save_file}.txt"
        ...
        #self.info(center_message("Finished Partial FedAvg."))
