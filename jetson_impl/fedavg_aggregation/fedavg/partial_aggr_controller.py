...
class PartialAggController(ModelController):
    ...
    def run(self) -> None:
        ...
        for self.current_round in range(self.start_round, self.start_round + self.num_rounds):
            ...
            save_path = f"/files/jetson_impl/fedavg_aggregation/fedavg/stats/{save_file}_params_{self.current_round}.pt" 
            print(f"Params save file: {save_path}", flush=True)
            torch.save(self.model_params, save_path)

            end_time=time.time()

        stats_save_path = f"/files/jetson_impl/fedavg_aggregation/fedavg/stats/training_stats_{save_file}.txt"
