...
def get_label_from_path(filepath):
    """
    Extracts the freeze rate from the filename.
    1. If filename contains 'fedavg' -> Freeze Rate: 0
    2. If filename contains '_act_XXX_' -> Extract rate (e.g. 095 -> 0.95)
    3. Else -> Freeze Rate: 0.9 (Default)
    """
...
def plot_comparison(all_baselines, metric_key, title, ylabel, save_dir, filename, conversion_func=None):
    ...
    # Sort labels numerically by the freeze rate value
    def sort_key(label):
        try:
...
def add_seconds_per_batch_metric(parsed_data, client_batches):
    ...
    for round_data in raw_times:
        ...
        for cl in round_data.items():
            t = cl[1] / client_batches[cl[0].split('-')[-1]]
...
def print_statistics(all_baselines):
    ...
    TARGET_REF = "Freeze Rate: 0.9"

    print(f"\n{'='*110}")
    print(f"FINAL STATISTICS: FREEZE RATE COMPARISON (vs {TARGET_REF})")
    print(f"{'='*110}")
...
if __name__ == "__main__":
    ...
    plot_comparison(parsed_baselines, 'losses', "Comparison: Training Loss by Freeze Rate", "Loss", output_dir, "compare_losses_freeze.png")
    plot_comparison(parsed_baselines, 'trainable_experts', "Comparison: Active Experts by Freeze Rate", "Experts", output_dir, "compare_experts_freeze.png")
    
    if client_batches:
        plot_comparison(parsed_baselines, 'seconds_per_batch', "Comparison: Training Time (Seconds/Batch)", "Seconds per Batch", output_dir, "compare_time_per_batch_freeze.png")
    else:
        plot_comparison(parsed_baselines, 'training_time', "Comparison: Training Time", "Time (min)", output_dir, "compare_time_freeze.png")

    to_MB = lambda x: x / (1024.0**2)
    plot_comparison(parsed_baselines, 'server_data', "Comparison: Server Data", "Size (MB)", output_dir, "compare_server_data_freeze.png", conversion_func=to_MB)
    plot_comparison(parsed_baselines, 'client_data', "Comparison: Client Data", "Size (MB)", output_dir, "compare_client_data_freeze.png", conversion_func=to_MB)
    plot_comparison(parsed_baselines, 'gpu_alloc', "Comparison: GPU Allocated", "VRAM (MB)", output_dir, "compare_gpu_alloc_freeze.png")
    plot_comparison(parsed_baselines, 'gpu_res', "Comparison: GPU Reserved", "VRAM (MB)", output_dir, "compare_gpu_res_freeze.png")
