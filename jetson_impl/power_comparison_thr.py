...
def get_label_from_path(filepath):
    ...
    Example input: power_boolq_03_act_thr_010.txt
    ...
    # 1. Handle FedAvg case (Threshold is effectively 0)
    if "fedavg" in filename:
        return "Threshold: 0"

    # 2. Regex to find 'act_thr_010', 'act_thr_05', etc.
    # Matches _act_thr_ followed by digits
...
def plot_bar_comparison(baselines_metrics, metric_key, title, ylabel, filename, save_dir, is_energy=False):
    ...
    # 1. Sort labels numerically based on the threshold value
    # Function to extract float from "Threshold: 0.10"
    def sort_key(label):
        try:
    ...
    # 4. Calculate Savings (Relative to Threshold: 0 / FedAvg if present)
    baseline_val = None
    if "Threshold: 0" in baselines_metrics:
        baseline_val = baselines_metrics["Threshold: 0"][metric_key]
    elif "Threshold: 0.0" in baselines_metrics:
        baseline_val = baselines_metrics["Threshold: 0.0"][metric_key]
    ...
    for bar, label, val in zip(bars, sorted_labels, values):
        ...
        # Ensure we don't compare baseline to itself for the percentage label
        if is_energy and baseline_val and (val < baseline_val) and ("Threshold: 0" not in label):
...
if __name__ == "__main__":
    ...
    for filepath in args.files:
        # Determine the label based on threshold
    ...
    plot_bar_comparison(
        baselines_metrics, 
        'total_energy_wh', 
        "Total Energy Consumption by Threshold", 
        "Energy (Watt-hours)", 
        "compare_energy_threshold.png", 
        output_dir, 
        is_energy=True
    )
    ...
    plot_bar_comparison(
        baselines_metrics, 
        'avg_power_w', 
        "Average Power Draw by Threshold", 
        "Power (Watts)", 
        "compare_power_threshold.png", 
        output_dir
    )
